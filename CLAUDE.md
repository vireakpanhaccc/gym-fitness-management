...
5. Configure **Fanout DNS** using a domain name (e.g. `aupp.com`) — implemented via a Kubernetes `Ingress` resource that routes based on host/path to the API Gateway (and potentially fans out to multiple services).
...
- **Ingress / Fanout DNS:** domain `aupp.com`, resolved locally via `/etc/hosts` → `minikube ip`. Ingress controller = `ingress-nginx` (enabled via `minikube addons enable ingress`).
...
- **Member B — API Gateway & Ingress (Fanout DNS):** Gateway Deployment/Service, Ingress YAML for `aupp.com`, enabling Minikube ingress addon, verifying all traffic funnels through the gateway and backend services aren't directly reachable externally.
...
- [ ] Final domain routing scheme: single host with path-based fanout (`aupp.com/gateway/...`), or multiple subdomains fanning out to the gateway?
